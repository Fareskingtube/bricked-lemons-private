  ...
  http.get("http://localhost:3000/api/products", ({ request }) => {
    ...
    if (search === "Lemon") {
      return HttpResponse.json({
        success: true,
        pagination: { totalItems: 2, currentPage: 1, totalPages: 1, limit: 10 },
        data: [
          { id: 1, name: "Mocked MSW Sour Lemon", price: 5.99, category: "Fruits" },
          { id: 2, name: "Mocked MSW Sweet Lemon", price: 2.99, category: "Fruits" }
        ]
      }, { status: 200 });
    }
