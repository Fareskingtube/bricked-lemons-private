...
process.env.JWT_SECRET = Buffer.from("test-secret-key-for-jwt-signing").toString("base64");
...
const mockBcryptHash = jest.fn() as unknown as jest.MockedFunction<(...args: any[]) => Promise<any>>;
const mockBcryptCompare = jest.fn() as unknown as jest.MockedFunction<(...args: any[]) => Promise<any>>;
const mockJwtSign = jest.fn() as unknown as jest.MockedFunction<(...args: any[]) => any>;
...
const { register, login, logout, getProfile } = await import("../controllers/authController.ts");
...
describe("Auth Controller - register", () => {
  ...
  it("should return 400 if a required field is missing", async () => {
    ...
    expect(jsonMock).toHaveBeenCalledWith({ message: "Please provide all required fields" });
  });
  ...
  it("should return 400 if the email is already in use", async () => {
    ...
    mockFindUnique.mockResolvedValue({ id: "existing-id", email: "alex@example.com" });
  ...
  it("should hash the password, create the user, set a cookie, and return 201", async () => {
    ...
    expect(mockCreate).toHaveBeenCalledWith({
      data: { username: "alex", email: "alex@example.com", password: "hashed-password" },
    });
...
describe("Auth Controller - logout", () => {
  it("should clear the token cookie and return 200", async () => {
    ...
    const res = { status: statusMock, cookie: cookieMock } as unknown as Response;
    ...
    expect(statusMock).toHaveBeenCalledWith(200);
    expect(jsonMock).toHaveBeenCalledWith({ message: "Logged out in successfully" });
  });
});
...
describe("Auth Controller - getProfile", () => {
  it("should return req.user as the response body", async () => {
    ...
    const req = { user: { id: "id1", username: "alex", role: "USER" } } as unknown as Request;
    const res = { json: jsonMock } as unknown as Response;

    await getProfile(req, res);

    expect(jsonMock).toHaveBeenCalledWith({ id: "id1", username: "alex", role: "USER" });
  });
});